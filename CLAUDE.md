...
**The already-seated guards are asymmetric, and have to be.** `surfaceBreakoutSet(eventId, breakoutSet)` (`breakout-actions.ts`) answers *which seats count against you*, for the already-seated and already-a-facilitator checks in `autoAssignRegistrantToBreakout` and `getCheckinBreakoutChoices`. The rule: **a seat counts if it is in the set being filled, or if it was made for today.** On a Collab day the cluster's set is today's and a member event's standing tables are dormant — not being run — so:
