...
**Two sets, side by side — a cluster never takes an event's tables away.** `resolvePoolScope(eventId).breakoutOwner` is **always** `{ eventId }`; `clusterBreakoutOwner` is `{ clusterId }` on a Collab and null otherwise. It used to be one field that *switched* to the cluster for every member event of a Collab, and the substitution reached every event-side surface at once — the registrant detail's placement list, the per-event pickers, the seating write, the session screen's tables, and an event's entire **Catch Mech history**, which is keyed to the tables it was recorded against. Joining a cluster silently emptied screens with years of work on them and leaving it was the only way back. An event does not stop being an event because it shares one Sunday.

Three ways a caller addresses the pair:

- **One set, chosen by the surface.** A write lands in exactly one, and the *event id cannot say which* — both answers are valid for the same event on a collab day. `BreakoutSet` (`"event" | "cluster"`) is the discriminator, resolved by `resolveSurfaceBreakoutOwner(eventId, set)`; an event's own forms and kiosk leave the default, the cluster's shared form and kiosk pass `"cluster"`. Deliberately a two-value literal and **never a caller-supplied owner id**: these travel from client components to public unauthenticated actions, and a person registered on that event is entitled to both of their own two sets, so nothing is escalated. `"cluster"` degrades to the event's set where no cluster set exists.
- **Both sets, for "where is this person sitting".** For READS only, and **ordered rather than unioned**: `getRegistrantBreakoutGroupName` asks the day's set first and falls back to the event's. Holding a seat in each is an ordinary state, not a fault — a recurring event's regular keeps their standing seat for the whole series while the day seats them at one of its own — so a single `findFirst` over an `OR` named whichever row Postgres reached first. A check-in confirmation can only usefully name the table being run in the room.
- **Both sets, for Catch Mech.** See below.

**The already-seated guards are asymmetric, and have to be.** `resolveSeatedScope(owner)` (`lib/events/pool-scope.ts`) answers *which seats count against you*; `surfaceBreakoutSet(eventId, set)` in `breakout-actions.ts` is the same rule reached from a surface rather than an owner. The rule: **a seat counts if it is in the set being filled, or if it was made for today.** On a Collab day the cluster's set is today's and a member event's standing tables are dormant — not being run — so:

- Filling the **day's** set (`"cluster"`): only the day's seats answer. **A standing seat is not an answer about today.**
- Filling an **event's own** set (`"event"`): both answer. Today's placement stands, and there is nothing to gain by moving someone into a table that isn't running.

Only the second direction was ever true. Applied symmetrically it broke the commonest collab day: `EventRegistrant` is one row per person per *series* and `BreakoutGroupMember` has no per-occurrence scoping, so a recurring event's regular holds a standing seat permanently — the day's kiosk found it, called them placed and skipped its step, and with `deferBreakoutToCheckin` holding registration off *because* the kiosk was asking, they finished the day seated nowhere and were shown a table that wasn't running. Holding a seat in each set is therefore ordinary, not a fault: the invariant is one seat per person **per owner**, which is what `personKeyFor` and `carryOverBreakoutGroups`' own `seated` set enforce.

In the `"event"` direction this is deliberately wider than the write — `assignBreakoutForRegistrant` scopes `current` to the surface's owner alone, because `current` also names the row to *move* for an explicit pick and a pick of an event table must not delete the day's seat. Combine the result with `AND`, never a spread: it carries an `OR` whenever both sets are in play.

**It governs the automatic routes, not the manual ones.** `autoAssignRegistrantToBreakout`, `getCheckinBreakoutChoices`, the bulk `autoAssignBreakouts`, and the event Breakouts page's "N unassigned" all read it — the last two must agree, since one describes what the other would do. Scoped on a bare owner, the bulk action read *every* person the day had seated as unassigned and seated the lot a second time in the event's dormant standing tables, in one click. The admin **add and transfer** screens deliberately keep the narrower per-owner question: they are the manual escape hatch, and holding a seat in each set is an ordinary state rather than a fault.
...
**`manualAssignOnly` is the narrow sibling of `isEnabled`.** Switching a group **off** closes every automatic *and* public route into it at once — it leaves the dropdown, the kiosk's browse list and the admin add/transfer screens together. `manualAssignOnly` closes only the automatic half: never suggested, never auto-assigned, still fully pickable by anyone who wants it. That is the shape of an overflow table held in reserve, a table for a named list, or a facilitator's group they intend to fill by hand. An explicit pick of one is **honoured**, at the form and at the door — a disabled group refuses a submitted pick outright, this one accepts it, because a pick is somebody choosing and the setting only suppresses the system choosing.

It is deliberately **not** a `where` fragment on the candidate query the way `ENABLED_BREAKOUT_WHERE` is. `suggestBreakoutGroup` and `breakoutPickerOptions` are fed by **one loaded set**, so the flag rides on the `BreakoutCandidate` and is applied inside `isEligible` alone — filtering it in SQL there would make it behave exactly like `isEnabled: false`, which is the thing it exists to avoid. Only `matchBreakoutGroups` filters on the column, via `AUTO_ASSIGNABLE_BREAKOUT_WHERE` (`lib/breakouts/candidate-pool.ts`), because it has no browse list beside it — every one of its callers is an automatic or suggestion surface. A held-back group still counts toward `totalGroups`, since it *is* offered; and it keeps its rank in the dropdown, because it holds real people and hiding it from `fillLevel` would distort the ordering of the rest.

Auto-assign never falls back to one: with every table held back, the registrant is left unseated for staff to place. That pairing — auto-assign on, nothing auto-assignable — suppresses the Breakout step in favour of a placement that cannot happen, so `breakoutPickerReadiness` carries `autoAssignableGroups` beside `enabledGroups` and both prerequisite chains name it ahead of the ordinary auto-assign notice.
...
**A kiosk is a page left open for hours, and its server props go stale under it.** The walk-in door and both check-in boards are opened once on a tablet and run for the length of an event. Everything they were server-rendered with ages: the door's breakout candidates (whose set is *gated* on which facilitators have checked in — designed to change through the first half hour), their headcounts and `isFull`, and the form config itself (`sectionBreakout`, `autoAssignBreakout`, the door's open state). **`revalidatePath` cannot fix this and never could** — it clears a server cache, while the stale copy is a React tree already mounted in a browser somewhere else. Only the client can ask again.

`useKioskRefresh` (`lib/hooks/use-kiosk-refresh.ts`) is the seam: `router.refresh()` re-runs the server component and hands down new props while leaving client state (a half-typed form, the step someone is on) untouched. Two moments, and both are needed:

- **Between people** — `handleReset` on the form and `reset()` on each board. That half of a reset is as necessary as clearing the fields; without it the second walk-in of the morning is offered the tables and counts as they stood at page load.
- **While idle** — a 60s poll, visibility-aware on both edges (never polls a backgrounded tab; catches up the moment it returns). This covers what reset cannot: the tablet opened at 8am whose first walk-in arrives at 9, long after the facilitators checked in.

`revalidatePath` still matters for the *next* render, and both owners now reach their public pickers — the cluster branch by **route** (`revalidatePath("/register/c/[token]", "page")`), because a day's public surfaces are keyed by `publicToken` while the function is handed an owner carrying the cluster's id. Create and delete pass `publicPickers` too: a table that just appeared or vanished is one the forms should stop or start offering.

**A kiosk with no network says so.** `OfflineNotice` (`components/offline-notice.tsx`) over `useIsOffline`. The PWA serves page documents `NetworkFirst` with a 24h cache — right for venue wifi, but offline the door renders perfectly from cache and gives no sign that a submit is going nowhere. It is **advisory, never a block**: only `navigator.onLine === false` is trustworthy (`true` says nothing about reachability — a captive portal is `true` all day), and greying out a working door on a false negative is worse than a submit that fails with a readable message. It sits in `FormShell`, which every step of the registration form returns through, so a mid-form drop is covered as well as one that predates the door opening. `useKioskRefresh` skips polling while offline and refreshes on the `online` edge, so the banner and the stale data behind it clear together.

**It fires only at a step where nothing is in progress** — the door's identify gate, a board's lookup screen. Never at `"form"`, even on a door configured without a mobile field where that is the resting step: `config` decides which questions render, so a refresh landing there would rearrange them under someone answering. Those doors still refresh on reset; they just don't poll. The PWA service worker doesn't defeat any of this — `@ducanh2912/next-pwa` handles RSC requests `NetworkFirst`, so a soft refresh reaches the network.
...
| Breakouts | Per event | **Cluster-owned, beside the member events' own — not instead of them.** Empty by default. `carryOverBreakoutGroups` copies a member event's tables in (optionally with rosters) |
...
**An event's Catch Mech is its own tables plus the day's tables it staffs — a union, never a swap.** There is no cluster-level Catch Mech. A cluster-owned table is followed up by the member event of whoever staffs it — `Volunteer.eventId` is required, so a facilitator belongs to exactly one member event, and `CatchMechSession` keeps its required `eventId` (that ministry) while pointing at the cluster's table. A table staffed from two ministries appears on both events' Catch Mech, which needs no special handling: `resolveCatchMechTargets` gives the linked DGroup only to the lead, and `hidesPerson` already scopes decisions per facilitator. A table with no staff is endorsed to nobody and is flagged on the cluster Breakouts page.

`lib/catch-mech/scope.ts` is the seam and returns two filters. `where` is the tables whose follow-up belongs to this event — `{ eventId }` OR the endorsed cluster tables. `seatedWhere` is wider: every table someone could be sitting at, the partner ministry's included, because "who is seated nowhere" must not count a person at their own table as unseated. **Never scope a Catch Mech query on a bare `eventId`**, and combine either filter with `AND` — the Collab branches are themselves `OR`s.

The Collab branch used to *replace* `{ eventId }` rather than union with it. Every Catch Mech read filters on `breakoutGroupId IN <scope>`, so joining a collab blanked the dashboard counts, all three status lists and the submissions log at once. Nothing was ever deleted — it was unreachable. `scripts/audit-catch-mech-scope.ts` is the read-only audit that tells the two apart.

**A substitute can stand in at a cluster-owned table.** `OccurrenceSubFacilitator` is keyed by `[occurrenceId, breakoutGroupId, role]` and carries no event of its own, so the row could always name one — what could not was the screen that writes it. The session page listed tables on a bare `eventId`, so a Collab day offered the member event's standing tables and none of the day's own. Both the page and `sub-facilitator-actions.ts` resolve through `resolvePoolScope` now, and the table may be in **either** set (`anyOwner` of the pair): a sitting can need a stand-in at the event's own standing table or at the day's, and a scope naming one leaves the other unstaffable. Which set the table came from then decides who may write it — `authorizeTable` requires the event's own breakout-write permission as a floor and *additionally* the cluster's when the table is cluster-owned, resolved from the row via `ownerOf` rather than assumed from the session. The **substitute** comes from `volunteerEventIds` — either ministry's roster, since a cluster-owned table can be staffed from either. The occurrence itself stays the member event's; a substitute stands in at one sitting of one event.
